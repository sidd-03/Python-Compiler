...
function    
        ...
        //printf("Function successfully parsed:\n"); 
                ...
                entry_t* entry = symbol_table_lookup(global_table, idFunc->value);
                 if (entry) {
                    std::cerr << "Error: Function " << idFunc->name << " already defined" << std::endl;
                }
                else {
...
function_call     
            ...
            $$ = new FunctionCallNode(idFunc->value + name , size,func_type);
            ...
            | PRINT '(' argsp ')' {
                ...
                $$ = new FunctionCallNode(name,size);
                ++n_nodes;
                if($3!=NULL){
                $$->add($3);
                }
...
assignment
        ...
        | ID '[' arith_expression ']' '=' arith_expression   {
                std::string name = "assignment" + std::to_string(n_nodes);
                ++n_nodes;
                std::string nname = "iden" + std::to_string(n_nodes);
                $1->node_number = n_nodes;
...
{
        int input_flag =0, output_flag = false, tac_output_flag = false, symtab_output_flag = false;
        ...
        for (int i=0; i< argc; i++)
        {
                ...
                if (strcmp(argv[i], "-help") == 0)
                {
                        ...
                        cerr<<"Sample: ./a.out -input input.txt -output output.dot -output3AC output3AC.txt -outputSymTab outputSymTab.csv\n";
                        cerr<<"The default input file is input.py, the default digraph output file is output.dot, the default 3AC output file is 3AC.txt and the default symbol table output file is symtab.csv.\n";
        ...
        yyparse(); // Redirect cout to the output file
        root->generate_quad();
